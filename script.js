...
async function displayProducts(filterCategory = 'all') {
    ...
    Object.values(grouped).forEach(group => {
        ...
        const hasImg = imageValue && imageValue.length > 5;
...
/* Admin Functions (Moved to local or calling admin.js) */
        ...
        if (loadedProducts.length > 0) {
            ...
            if (products.length > 0) {
                ...
                if (isFirstLoad && hasBeenInitialized !== 'true') {
                    ...
                    await saveProducts();
                    // Mark as initialized so we don't save defaults on every load
